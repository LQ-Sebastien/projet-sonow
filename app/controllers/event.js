...
module.exports = {
  ...
  async addToBookmarks(req, res) {
    const result = await eventDataMapper.pinEvent(
      req.body.user_id,
      req.body.event_id
    );
    ...
    if (!result) {
      ...
      return res
        .status(200)
        .json({ result, message: "Event added to bookmarks succesfully" });
    }
  },

  //Méthode qui permet de supprimer un évènement aux favoris de l'utilisateur.
  async delToBookmarks(req, res) {
    const result = await eventDataMapper.unpinEvent(
      req.body.user_id,
      req.body.event_id
    );
    ...
    if (!result) {
      ...
      return res
        .status(200)
        .json({ result, message: "Event deleted from bookmarks succesfully" });
    }
  },

  //Méthode qui permet de s'identifier comme participant à un évènement.
  async addAttendEvent(req, res) {
    const result = await eventDataMapper.pinAttendEvent(
      req.body.user_id,
      req.body.event_id
    );
  ...
  async delAttendEvent(req, res) {
    const result = await eventDataMapper.unpinAttendEvent(
      req.body.user_id,
      req.body.event_id
    );
  ...
  async updateEvent(req, res) {
    ...
    const savedEvent = await eventDataMapper.update(
      req.params.event_id,
      req.body
    );
  ...
  async deleteEvent(req, res) {
    ...
    return res
      .status(200)
      .json({ code: 200, message: "Event has been deleted" });
  },
};
