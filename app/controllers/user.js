...
module.exports = {
    ...
    if (user) {
      bcrypt.compare(password, user.password, function (err, result) {
        if (result) {
          ...
          return res
            .status(200)
            .json({ accessToken, refreshToken, user: req.session.user });
        } else {
          return res
            .status(403)
            .json({
  ...
  async followUser(req, res) {
    const follow = await userDataMapper.pinFollowUser(
      req.body.user_follower,
    ...
    if (!follow) {
      ...
      return res
        .status(200)
        .json({ follow, message: "User followed succesfully" });
    }
  },

  //Méthode qui permet de se désabonner de l'activité d'un ami.
  async unfollowUser(req, res) {
    const unfollow = await userDataMapper.unpinFollowUser(
      req.body.user_follower,
      req.body.user_followed
    );
    ...
    if (!unfollow) {
      ...
      return res
        .status(200)
        .json({ unfollow, message: "User unfollowed succesfully" });
    }
  },
  ...
  async getFollowers(req, res) {
    const result = await userDataMapper.findFollowersByUserId(
      req.params.user_id
    );
  ...
  async getFollowed(req, res) {
    const result = await userDataMapper.findFollowedByUserid(
      req.params.user_id
    );
